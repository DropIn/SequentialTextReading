...
template<typename T>
void ShowMathGLDataSorted(const std::vector<T>& values, const int* cutoffLine = NULL, const char* name = NULL) {
#ifdef HAVE_MATHGL
    std::vector<T> sorted(values);
    std::sort(sorted.rbegin(), sorted.rend());
    ShowMathGLData(sorted,cutoffLine,name);
#endif
}

template<typename T>
void ShowMathGLCompareCurves(const std::vector<Point_<T> >& a_canon, const std::vector<Point_<T> >& b_canon, const std::vector<double>& a_sig, const std::vector<double>& b_sig, double cc) {
#ifdef HAVE_MATHGL
    mglGraph gr;
    gr.SubPlot(2, 1, 0, "");
    ...
    std::stringstream ss; ss << "cross correlation " << cc;
...
void ShowMathGLCurve(const vector<Point_<T> > a_canon, const std::string& title)
{
	...
	Mat img(gr.GetHeight(),gr.GetWidth(),CV_8UC3,(void*)gr.GetRGB());
	std::stringstream ss1; ss1 << "MathMLCurves " << mgl_id; mgl_id++;
	imshow(ss1.str(), img);
#endif
}
